...
struct CROWDYSERVICES_API FCrowdyWildcardStruct { GENERATED_BODY() };
...
public:
	...
	void K2_PushState(const FCrowdyWildcardStruct& State, UObject* Subject);
	...
	void K2_PullState(FLatentActionInfo LatentInfo, UObject* Subject,
	...
	{
		InternalPushState(T::StaticStruct(), &Data, Subject);
	}

	/** Pull a typed struct asynchronously. Callback fires on the game thread. */
	template<typename T>
	void PullState(UObject* Subject, TFunction<void(bool bSuccess, const T&)> Callback)
	{
		InternalPullStateCallback(T::StaticStruct(), Subject,
...
private:
	...
	struct FPersistenceTypeInfo
	{
		...
		int64 ChunkX     = 0;
		/** True unless the struct is tagged meta=(CrowdyPersistent, CrowdySingleton). Instanced is the default. */
	...
	void InternalPullStateCallback(UScriptStruct* StructType, UObject* Subject,
	                               TFunction<void(bool, const TArray<uint8>&, UScriptStruct*)> Callback);
