...
namespace CrowdyNetTrace
{
	...
	// crowdy.query.trace - GraphQL: query dispatch and response.
...
// Fine-grained CPU trace scopes inside payload decode, off by default and readable from any thread.
//
// These sit INSIDE scopes that are themselves measured, and a scope costs two timestamps per message, so
// leaving one on shifts the enclosing scope's own number. Turn it on only for the run that needs the
// split, and say in the reading which runs carried it.
namespace CrowdyNetProfile
{
	// crowdy.serialize.scopes - a scope around the reflective struct walk inside actor-state decode.
	CROWDYNET_API bool DecodeScopes();
}
