...
namespace CrowdyApplyEffectNodePins
{
	...
	// String magnitude values a rebuild would reset: the schema keeps an old value only when it differs ignoring case.
	CROWDYNODES_API TMap<FName, FString> CaptureCaseOnlyMagnitudeValues(const TArray<UEdGraphPin*>& OldPins);

	// Put each captured value back on the rebuilt pin of the same name.
	CROWDYNODES_API void RestoreCaseOnlyMagnitudeValues(const TMap<FName, FString>& Values, const TArray<UEdGraphPin*>& Pins);
}
